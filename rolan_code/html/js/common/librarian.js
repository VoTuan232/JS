...
function Librarian(config) {
	...
	if (config.hasOwnProperty('extRows')) {
		for (var row = 0; row < config.extRows; row++) {
			this.cells[this.rows + row] = new LibrarianCell(config);
			for (var n = 0, num = blockSet.length; n < num; n++) {
				var bid = config.extBlock(blockSet[n]);
				this.cells[this.rows + row].paramSet[bid] = Parameter.paramSet[_bid(bid, row)];
			}
		}
	}

	//todo fix this.cells[0].paramSet[prop] undefined
	this.paramSet0 = {}; /* for factory */
	for (var prop in this.cells[0].paramSet) {
		if(this.cells[0].paramSet[prop]) {
...
Librarian.prototype.read = function(rows) {
	...
	while ((row = rows.shift()) !== undefined) {
		for (var n = 0, num = blockSet.length; n < num; n++) {
			...
			var index = row;
			if (row >= this.rows) {
				bid = this.config.extBlock(bid);
				index = row - this.rows;
			}
			var b = Parameter.getblock(_bid(bid, index));
...
Librarian.prototype.temporaryWrite = function(cell, row) {
	...
	if (this.config.exclusionSet) {
		exclusionSet = this.config.exclusionSet;
	}
	for (var prop in temporarySet) {
		var b = Parameter.getblock(temporarySet[prop]);
		var d = cell.paramSet[prop];
		if (exclusionSet.indexOf(prop) >= 0) {
			Parameter.initialize(temporarySet[prop]);
			d = b.data;
		}
		setTasks(tasks, b, d);
	}
