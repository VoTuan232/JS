...
$(function () {
	...
	$(document).on('keydown', responders, function (e) {
		...
		if ($('#Numpad').css('display') == 'block') return;
		var _$ = $(this);
		var v = _$.val();
	...
	$(document).on(pointer.click, '.elf-step-value', function (e) {
		e.stopPropagation();
		e.preventDefault();
		var item = _items[$(this).attr('id')];
	...
	$(document).on(pointer.click, '.elf-popup', function (e) {
		e.stopPropagation();
		e.preventDefault();
		var item = _items[$(this).attr('id')];
		if (item.popup == '--close') {
	...
	window.parameter_load = parameter_load;
});
