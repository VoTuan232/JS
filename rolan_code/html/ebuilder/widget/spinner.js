...
(function(window) {
	...
	widget['spinner'].properties = [ 'nth-power', 'format', 'unformat', 'stringer' ]
	...
	widget['spinner'].create$ = function(item) {
		...
		var unformat = item['unformat'] ? item['unformat'] : '';

		var _$ = create$(
			'<div id="%id" class="%class">' +
				'<input type="text" min="%min" max="%max" value="%value" power="%power" format="%format" unformat="%unformat" />' +
