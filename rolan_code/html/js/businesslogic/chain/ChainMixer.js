...
BLOCK_WIDTH BRANCH_POINT_LINE_TYPE
*/
...
ChainMixer.prototype.updateLines = function (currentX, splitterX, blockIdArray) {
    ...
    if (dividerIndex + 1 === splitterIndex && splitterIndex + 1 === mixerIndex) {
        ...
        if (this.blockModelMap[CHAIN_BLOCK_ID_MAP.divTemp1].aBranchChildDividerIds.length === 0 && 
            this.blockModelMap[CHAIN_BLOCK_ID_MAP.divTemp1].bBranchChildDividerIds.length > 0 && 
            this.mixer.id === CHAIN_BLOCK_ID_MAP.balancer3) {
                this.lines = [].concat(
                    this.getLinesSpecial(),
                    this.getJunctionLine(blockIdArray),
                    this.getBBranchLine(),
                    this.getPaddingLines(currentX, splitterX, BLOCK_WIDTH)
                );
        } else {
            this.lines = [].concat(
                this.getABranchLine(),
                this.getJunctionLine(blockIdArray),
                this.getBBranchLine(),
                this.getPaddingLines(currentX, splitterX, 0)
            );
        }
...
ChainMixer.prototype.getPaddingLines = function (currentX, splitterX, special) {
    ...
    if (diff < 0) {
        ...
        x = splitterX;
        y = this.y - this.aBranchHeight - special;
